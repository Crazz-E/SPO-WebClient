 ...
 * `RDOFavoritesMoveItem` answers a boolean — false when `itemPath` does not resolve, is the
 * root, or `destPath` does not resolve, or `destPath` sits inside `itemPath`'s own subtree
 * (`TFavorites.RDOMoveItem`, `Kernel/Favorites.pas:239-247`). Any non-zero ordinal is true on
 * the wire.
 *
