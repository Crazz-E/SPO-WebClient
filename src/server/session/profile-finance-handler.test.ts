...
/**
 * A tax account (`Obj.AccountIsTax`, :167/:179): the same `<tr>` `plRow` emits,
 * with the tax cells appended before `</tr>` — captions for a level-2 header
 * (:167-178), split figures for a deeper row (:179-194).
 */
...
describe('fetchProfitLoss', () => {
  ...
  it('a tax section carries its Town/IFEL split and every other amount stays byte-identical', async () => {
    const fake = makeWebCtx();
    fetchAsp(fake).mockResolvedValue([
      plRow(0, 'Net Profit (losses)', '$1,000,000', '3,10,-20,30'),
      plRow(1, 'Income', '$2,000,000'),
    ...
    // Verbatim from the first test — the tree shape and every existing amount
    // stay byte-identical on a page that also contains tax rows.
    expect(root).toMatchObject({ label: 'Net Profit (losses)', level: 0, amount: '1000000', chartData: [10, -20, 30] });
    ...
    expect(residentials.children![0]).toMatchObject({ amount: '500000', chartData: undefined });
    expect(expenses).toMatchObject({ label: 'Expenses', level: 1, amount: '-1000000' });
    const [salaries, taxes] = expenses.children as ProfitLossNode[];
    ...
    expect(importTax).toMatchObject({ isTax: true, amount: '-50000', secAmount: '-20000' });
  ...
  it('a tax row with only one split figure is not tax', async () => {
    ...
    // Drop the second split cell so only one figure survives.
    const truncated = row.substring(0, row.lastIndexOf('<td style="padding-left: 20px"'))
      + `${T(2)}</tr>`;
    fetchAsp(fake).mockResolvedValue(plRow(0, 'Net', '$1') + '\n' + truncated);
    const data = await fetchProfitLoss(fake.ctx);
    expect(data.root.children![0]).toMatchObject({ amount: '300000' });
    expect(data.root.children![0].isTax).toBeUndefined();
  });

  it('a tax row truncated before its </tr> still reads its split, to the end of the page', async () => {
    const fake = makeWebCtx();
    const row = plTaxRow(3, 'Income tax', '$300,000', '$200,000', '$100,000');
    fetchAsp(fake).mockResolvedValue(
