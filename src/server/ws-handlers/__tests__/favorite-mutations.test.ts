...
import { handleFavoriteAdd, handleFavoriteDelete, handleFavoriteRename, handleFavoriteFolderCreate, handleFavoriteMove } from '../misc-handlers';
...
describe('handleFavoriteFolderCreate', () => {
  it('forwards the parent path and name and echoes the assigned id', async () => {
    const r = createCtx('createFavoriteFolder', { success: true, id: 9 });

    await handleFavoriteFolderCreate(r.ctx, msg(WsMessageType.REQ_FAVORITE_FOLDER_CREATE, { parentPath: '', name: 'Farms' }));

    expect(r.fn).toHaveBeenCalledWith('', 'Farms');
    expect(r.sent).toEqual([{
      type: WsMessageType.RESP_FAVORITE_FOLDER_CREATE, wsRequestId: 'req-1', success: true, id: 9,
    }]);
  });

  it('copies a refusal through verbatim', async () => {
    const r = createCtx('createFavoriteFolder', { success: false, message: 'Nope.' });
...
describe('handleFavoriteMove', () => {
  it('forwards the item path and destination path, in that order', async () => {
