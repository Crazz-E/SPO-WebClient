...
import {
  ...
  handleFavoriteFolderCreate, handleFavoriteMove,
} from '../misc-handlers';
...
describe('handleFavoriteFolderCreate', () => {
  it('forwards the parent path and the name, and echoes the assigned id', async () => {
  ...
  it('copies a refusal through verbatim — it never becomes an OK', async () => {
    ...
    expect(r.sent[0].success).toBe(false);
    expect(r.sent[0].message).toBe('The server refused to create this folder.');
    expect(r.sent[0].id).toBeUndefined();
  });
...
describe('handleFavoriteMove', () => {
  it('forwards the item path and the destination, in that order', async () => {
