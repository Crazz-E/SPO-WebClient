...
describe('createFolder', () => {
  it('sends the parent path and name, then re-reads the list the server now holds', async () => {
    const { ctx, sendRequest, sendMessage, showNotification } = makeCtx({ success: true, id: 9 });
...
describe('moveFavorite', () => {
  it('sends the item path and destination path, then refetches', async () => {
    const { ctx, sendRequest, sendMessage, showNotification } = makeCtx({ success: true });
