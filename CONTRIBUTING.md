...
`docs`; `test`, `chore`, `build` are dropped), generated by `node scripts/release.js
<version>`. Nobody edits `CHANGELOG.md` by hand in a feature branch — write a good PR title
instead.
