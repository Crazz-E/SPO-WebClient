...
| B5 | Confirmation des actions destructives / coûteuses | ✅ **fait (socle-2 → PR #79)** — Dialog branché sur démolir, la dépense de construction (T1), la déconnexion fournisseur/client (T3), la pose de route (dépense : tuiles, coût, trésorerie après — T8 PR #66), la démolition de route (destructif, clic et zone — T8) et la **rétrogradation** (PR #79 : Dialog destructif dans `upgradeBuildingAction`, l'envoi ne part qu'à la confirmation — un niveau payé disparaît sans retour) ; état initial : `requestConfirm` : démolir seulement | — | — |
