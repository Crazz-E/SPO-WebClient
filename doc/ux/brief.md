...
- **Tablette 768–1023 px** : **le modèle mobile** (décision du porteur, lot g PR #82) — feuille basse + barre d'onglets, avec des plafonds tablette (tuiles ≤ 120 px réparties sur la largeur, pastille de recherche ≤ 480 px, colonne de contenu de la feuille ≤ 640 px, clusters d'action ≤ 600 px).
...
| #69 | **Feuille flottante** — desktop : cadre 472 px (420 en desktop étroit), 64 px du haut, 16 px des bords, arrondi + ombre, la carte reste visible autour ; la barre de commande, le rail et la pastille se décalent (rail pour toute surface, pastille dégagée de la minicarte) ; tablette : panneau ancré conservé (remplacé au lot g, #82) | handoff 00 §3.2 |
...
| #74 | **Correctif (hors refonte)** — l'onglet Approvisionnements de l'inspecteur répondait « No supply inputs » sur **tous** les bâtiments : `SUPPLIES_GROUP` déclare `ObjectId`, qui est aussi une propriété d'en-tête, donc la passerelle publiait `groups['supplies']` et le client en déduisait la porte déjà chargée. Une seule liste (`isGateTab`) des deux côtés du socket ; les planches T3 disent enfin le vrai | — |
| #78 | **N6/N7 — déplacement + rotation** : pan au glisser **gauche** partout sauf route / zonage / démolition (clic différé au relâchement, seuil Manhattan 8 px = `cMinDrag` Voyager, `GameControl.pas:533-597`) ; flèches clavier = pan continu (demi-écran/s, diagonales, rAF — ajout assumé, Voyager les avait commentées) ; défilement au bord **écarté** (fenêtré + HUD sur les bords) ; rotation : deux boutons RightRail « Rotate view (Q) / (W) », Q/W dans `useKeyboardShortcuts` (Q quitte le renderer), « Rotate view » dans More › Map Controls sur mobile ; aria de `PlacementHUD` corrigé (H3) ; `key-binding-registry.ts` supprimé (décision inverse du handoff §4.3) | N6, N7, H3 |
| #79 | **B5 — rétrogradation confirmée** : « Downgrade » passe par le Dialog socle-2 (`requestConfirm`, kind destructive) dans `upgradeBuildingAction` — l'envoi ne part qu'à la confirmation, comme démolir et les routes ; un niveau payé disparaît sans retour. La QA live a révélé que le contrôle `UpgradeActions` (boutons Upgrade / Downgrade / Stop) n'était **rendu nulle part** — son `rdoName` figurait dans `HIDDEN_PROPERTY_NAMES` et le filtre par nom écartait le contrôle avec la valeur brute ; corrigé (exception par type), le contrôle affiche le niveau, la ligne « Upgrade Cost » reste (le widget ne montre pas le coût) | B5 |
| #82 | **Lot g — la tablette rejoint le modèle mobile** (décision du porteur ; détails instruits par agent UX) : le shell mobile monte sous 1024 px (`MobileShell` : `!isDesktop`), le panneau ancré 360 px et ses décalages HUD sont supprimés ; plafonds tablette — tuiles ≤ 120 px réparties bord à bord (pouces aux coins), feuille bord à bord avec colonne de contenu ≤ 640 px centrée, pastille de recherche ≤ 480 px, PlacementHUD / barre de mode ≤ 600 px centrés ; la minicarte ancrée suit la même frontière | tablette |
| #80 | **H6 + N10** (décisions instruites par agent UX) — H6 : « My facilities » en trois sections honnêtes (Losing money / Status unknown / Operating, croisement favoris × bit `alert` des bâtiments chargés, zéro RDO), tag Debt cliquable (StatusPill) et tag Debt tapable dans la barre d'info mobile ; N10 : « Pick on map » au pied du picker fournisseur — un seul mode connect à deux origines, décrit par la barre de mode (desktop + mobile), pile de surfaces masquée sans destruction pendant le mode, sortie Cancel/Échap, retour au picker intact | H6, N10 |

**Tous les lots du plan sont portés**, et la barre de mode mobile avec (#72). Reste, hors lot : N5 chat (linkification des coordonnées). N6/N7 et H3 sont fermés (#78), B5 (#79), H6 et N10 (#80), la tablette (#82) ; N4 serveur et H7 exact sont instruits en tickets RDO (OB-33 / OB-34, hors UI).

**Audit de clôture (24/08)** : chaque élément visuel des canevas est branché sur une fonction existante — les tables §9 de [missing-features](missing-features.md) sont à jour, chaque lot #78–#80 a été vérifié en L3 live (desktop et mobile), et ce que le code ne porte pas encore est tracé (tickets OB-33/OB-34, tablette, N5). Les trois canevas (Direction 🧭, Système 🧩, Flux 🗺️) portent une note d'état alignée sur ce commit.
